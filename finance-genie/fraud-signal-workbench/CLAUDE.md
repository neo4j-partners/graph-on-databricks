# Fraud Signal Workbench Development Guide

Full-stack Databricks App built with apx, React, Vite, and FastAPI. The repository
folder is `fraud-signal-workbench`; the internal Python package and stable
deployed resource identifiers remain `graph_fraud_analyst` and
`graph-fraud-analyst` for compatibility.
...
| `Dependencies.Neo4j` | `neo4j.Driver` | Neo4j sync driver against the bound Aura instance. Connection-pooled across requests. Use as `with driver.session() as session: ...`; FastAPI runs the sync handlers in a thread pool. |
