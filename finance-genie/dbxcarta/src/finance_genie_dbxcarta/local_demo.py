...
Runs entirely from published dbxcarta (dbxcarta-client[graph] + dbxcarta-core);
it imports no dbxcarta source. Retrieval and prompt assembly come from the
shared client seam so the demo cannot drift from the evaluation harness.
"""
...
# The bundled questions.json and the standalone .env both live at the consumer
# root, two levels up from this module, the same place the dbxcarta preset reads
# questions.json from. parents[2] resolves to finance-genie/dbxcarta/
# (local_demo.py -> finance_genie_dbxcarta -> src -> dbxcarta).
_CONSUMER_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_QUESTIONS = _CONSUMER_ROOT / "questions.json"
# Anchor .env lookup to the consumer root so the demo never inherits another
# project's .env.
DEMO_ENV_FILE = _CONSUMER_ROOT / ".env"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="finance-genie-dbxcarta demo",
