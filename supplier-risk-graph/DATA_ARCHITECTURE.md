...
The demo models a global beverage producer. Its suppliers specialize by subcategory (glass bottles, malt, hops, cans, labels, and the tiers behind the bottles, running from feedstock through the furnaces to the processors), and its customers are the drinks trade: distributors, wholesalers, supermarket groups, and bar and hotel chains.
...
Cascade Glassworks is a mid-tier raw-glass supplier. Its own risk score sits in the middling band, below the risk threshold, so no risk-score sort ever surfaces it. Yet the Americas business unit depends on it disproportionately, because its five clean tier-1 bottle suppliers all buy from a tier of glass processors, and every commodity-carrying glass path back through that tier arrives at Cascade. If Cascade fails, every glass path into the Americas fails with it, while the other four business units still reach a furnace of their own.

The graph engine surfaces Cascade with betweenness centrality over the multi-tier supply graph, the metric behind the **Critical Supplier** term. The supplier network is several regional clusters, sized by `SUP_CLUSTERS`, each webbed internally with a spanning tree plus chords and joined to the others by cross-cluster bridges. Every bridge is a different supplier and none of them trades glass. Cascade is not one of those bridges and is not a cut vertex: the build asserts that removing it leaves the network in one component. It earns its score by position instead, buying feedstock from vendors spread across the clusters and selling down through the processor tier to the bottle makers, so it sits between a large upstream population and a large downstream one. See `make_supply_relationships` in the generator and the constants block above it.

The exposure behind it is the Americas' recognized revenue for the most recent full calendar quarter, which is revenue that stops when Cascade stops rather than spend attributable to Cascade. The current figure is `story1_hidden_glassworks.bu03_last_quarter_revenue` in `data/ground_truth.json`, for the quarter named in `last_quarter`; it moves with every regeneration, so read it there rather than from this page.

Counting connections does not find Cascade. The build asserts that the most connected supplier is somebody else, and `report_degree_overlap` in `gds.py` prints how far the two rankings diverge on the realized data rather than asserting that they do. Cascade also owns no row in the supplier-to-unit bridge table, so nothing a region-scoped query reads names it, and it sells to nobody the Americas buys from directly, so one hop out from that unit's own suppliers stops at the processor tier. The narrow waist is not the busy node, which is the entire reason the metric has to be a shortest-path computation.
...
The scoping is what makes the figure attributable to a name rather than to reachability in general. A supply path counts only when every supplier along it trades in a subcategory of the commodity at risk, which the generator's `COMMODITY_SUBCATEGORIES` dict defines and `guard.py` reads through its `commodity_subcategories` helper. Without that scoping the background network's cross-cluster bridges would make almost every business unit reachable from almost every supplier, and the measure would return a number that no single supplier's failure explains. With it, the bridges can carry freight, equipment and ingredients without carrying glass, so the background stays structurally rich and the exposure stays leak-free.
...
- **Why it matters:** a plain risk-score filter only finds individually risky suppliers. Betweenness finds the supplier that a disproportionate share of the commodity-carrying paths into a business unit run through. Cascade sits between the feedstock vendors that supply it and the processor tier that carries its glass down to the Americas' five clean tier-1 bottle makers, so a large upstream population reaches a large downstream one through it while its own risk score stays middling. The score is what the governed percentile is compared against; it is not a ranking the demo depends on winning.
