...
- **`CLASSIFIED_AS`:** records a classification with provenance. The four column-findable terms carry these edges, and Critical Supplier now does too, written by `gds.py` from the governed threshold as a stand-in for a production batch job, with its cohort derived rather than named by hand. Ownership Risk alone carries none and is resolved live.
