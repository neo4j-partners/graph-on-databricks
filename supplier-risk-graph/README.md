...
- **The two graph-native terms have no lakehouse column, and their governing cutoff lives only in the graph, over a GDS metric BI cannot compute.** That is exactly why the lakehouse-only engine cannot resolve them, and it is the whole payoff. The graph resolves each from its governed definition and its graph cutoff, then walks the connections live.
