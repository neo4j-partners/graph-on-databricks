...
The three graph-native terms are not written here, because their cohorts do not
exist until gds.py has scored the networks. Critical Supplier is labelled by
...
Risky Customer is labelled from the governed kNN screen and receives explicit
delinquent-neighbour evidence. Ownership Risk still carries no CLASSIFIED_AS
edges at all.
...
All three terms carry a SCORED_BY edge to a GraphMetric node naming the property
...
rebuilt around the three stories and includes the SupplyRelationship entity,
...
REL_SPECS = [
    ...
    # CLASSIFIED_AS spec per label. The graph-native terms are never planted as
    # edges. gds.py derives Critical Supplier and Risky Customer after scoring;
    # Ownership Risk remains a live traversal.
